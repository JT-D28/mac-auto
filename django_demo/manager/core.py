...
def gen_third_invoke_url(planid):
	taskid=gettaskid()
	callername='%s_%s'%('jenkins',str(datetime.datetime.now()))
	mwstr='callername=%s&taskid=%s&planid=%s'%(callername,taskid,planid)

	return '%s/manager/third_party_call/?v=%s'%(settings.BASE_URL,EncryptUtils.base64_encrypt(EncryptUtils.des_encrypt(mwstr)))
...
class BusinessDataEncoder(XJsonEncoder):
	def __init__(self,**args):
		super(BusinessDataEncoder,self).__init__(['id','businessname','db_check','itf_check','params'],**args)
		...
		for x in L:
			...
			try:
				...
				for step in list(models.Step.objects.all()):
					businessids=[sb.id for sb in list(step.businessdatainfo.all())]
					#print(businessids)
					idd=x.get('id')
					try:
						idd=int(idd)

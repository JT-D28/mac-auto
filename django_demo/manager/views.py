...
from .core import *
from .invoker import *
from .context import querytestdata,gettestdatastep,mounttestdata,gettestdataparams,queryafteradd as qa,queryafterdel as qd,queryafteredit as qe
import json,threading,operator,xlrd,base64,traceback,warnings
...
def querydb(request):
	...
	if searchvalue:
		print("变量查询条件=>")
		res=list(DBCon.objects.filter(Q(description__icontains=searchvalue)|Q(dbname__icontains=searchvalue)|Q(host__icontains=searchvalue)|Q(port__icontains=searchvalue)|Q(kind__icontains=searchvalue)))
	else:
...
def queryvar(request):
	...
	# print('page=>',request.POST.get('page'))
	# res=None
	if searchvalue:
		print("变量查询条件=>")
		res=list(Variable.objects.filter(Q(description__icontains=searchvalue)|Q(key__icontains=searchvalue)|Q(value__icontains=searchvalue)|Q(gain__icontains=searchvalue)))
	else:
...
def queryitf(request):
	...
	if searchvalue:
		print("变量查询条件=>")
		res=list(Interface.objects.filter(Q(description__icontains=searchvalue)|Q(key__icontains=searchvalue)|Q(value__icontains=searchvalue)))
	else:
...
def queryonecase(request):
	...
	res=None
	try:
		res=Case.objects.get(id=request.POST.get('id'))
		pass
	except:
		code=1
		msg='查询异常[%s]'%traceback.format_exc()
...
def delcase(request):
	...
	try:
		for i in ids:
			...
			plan_all=list(Plan.objects.all())
			related_plans=[plan for plan in plan_all if case in list(plan.cases.all())]
			if len(related_plans)>0:
				code=1
				msg='用例已被计划[%s]引用 请先消除引用'%related_plans[0].description
			else:
				case.delete()
				#消除与包含步骤的引用关系
				Order.objects.filter(main_id=i,kind='step').delete()
				msg='删除成功'
	except:
		code=1
		msg="删除失败[%s]"%traceback.format_exc()
...
def transform(request):
	try:
		...
		if FILE_COUNT!=2:
			return JsonResponse(simplejson(code=101,msg='转换需要配置文件(Config.xlsx)和用例文件 实际上传%s个文件'%FILE_COUNT),safe=False)
...
def queryoneplan(request):
	...
	res=None
	try:
		res=Plan.objects.get(id=request.POST.get('id'))
		pass
	except:
		code=1
		msg='查询异常[%s]'%traceback.format_exc()
...
def delplan(request):
	...
	try:
		for i in ids:
			i=int(i)
			Plan.objects.get(id=i).delete()
			##消除与包含用例的映射关系
			Order.objects.filter(main_id=i,kind='case').delete()
		msg='删除成功'
...
def queryresultdetail(request):
	...
	if searchvalue:
		print("变量查询条件=>")
		res=list(ResultDetail.objects.filter(Q(description__icontains=searchvalue)|Q(key__icontains=searchvalue)|Q(value__icontains=searchvalue)))
	else:
...
def querystep(request):
	...
	if searchvalue and main_id is None:
		...
		# 	res=res+r
		res=res+list(Step.objects.filter(Q(description__icontains=searchvalue)|Q(step_type__icontains=searchvalue)))
...
def querymailconfig(request):
	...
	if searchvalue:
		print("变量查询条件=>")
		res=list(MailConfig.objects.filter(Q(description__icontains=searchvalue)|Q(rich_text__icontains=searchvalue)|Q(to_receive__icontains=searchvalue)|Q(cc_receive__icontains=searchvalue)))
	else:

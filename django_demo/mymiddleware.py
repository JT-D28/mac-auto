...
class Interceptor(MiddlewareMixin):
	...
	def  _repeat_check(self,request):
		...
		if simplename.startswith('add'):
			key=simplename.replace('add','').replace('edit','').lower()
			#print('key=>',key)
			mkey=getkey(_meta, key)
			if mkey:
				actionV=request.GET.get(_meta[mkey])
	...
	def _session_check(self,request):
		...
		session校验
		'''
		_meta=('/manager/addstep/','/account/login/','/manager/querytaskdetail/')

...
class Mysqloper:
    ...
    def db_execute(self, sql,taskid=None,callername=None):
        ...
        try:
            ...
            ##查询sql时候
            if re.match(r'(select).*(from).+(where){0,1}.*', sql.lower()) or re.match(r'(select)',sql.lower()):
                # for cs in range(15):
                data = cur.fetchall()
                data=list(data)
            ...
            msg="[<span style='color:#009999;'>%s</span>]执行sql <span style='color:#009999;'>%s</span> 结果为 <span style='color:#009999;'>%s</span>"%(conname,sql,sqlresult)
            #print(msg)

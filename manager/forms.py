...
class InterfaceForm(forms.Form):
	...
	content_type=forms.ChoiceField(choices=(('urlencode','xxx-www-form-urlencode'),('json','application/json'),('xml','application/xml')))
...
class StepForm(forms.Form):
	...
	url=forms.CharField(label='url',widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': "Username",'autofocus': ''}))
	method=forms.ChoiceField(label='method',choices=(('get','GET'),('post','POST')))
	content_type=forms.ChoiceField(label='content-type',choices=(('json','application/json'),('xml','application/xml'),('urlencode','x-www-form-urlencode')))
	version=forms.CharField(label='version',widget=forms.TextInput(attrs={'class': 'form-control'}))
	body=forms.CharField(label='body',widget=forms.TextInput(attrs={'class': 'form-control'}))

...
class MainSender:
	...
	def gethtmlcontent(cls,taskid,rich_text):
		...
		for case in data['cases']:
			...
			steps=case['steps']
			for steps_temp in steps.values():
				print('nnufa=>',steps_temp)
		...
		try:
			...
			for dwb in self.data_workbook:
				...
				for rowdata in act_data:
					...
					if not rowdata['参数值'].__contains__('：'):
			...
			for rowdata in var_sheet:
				...
				if _is_function_call(gain):
					...
					if methodname not in all_function_name:
						print('gin=>',gain)
						return ('fail','变量定义页没定义函数[%s],请先定义'%funcname)
				...
				for rowdata in v:
					...
					if rowdata['参数值'].__contains__('：'):
						...
						if self.is_xml:
							...
							if basic_config:
								step.url="%s%s"%(basic_config.get('host',''),detail_config.get('path',''))
...
class DataMove:
	...
	def export_plan(self,planid,export_flag,version=2):
		...
		return self._export_plan_new(planid,export_flag) if version==2 else self._export_plan_old(planid,export_flag)
	...
	def import_plan(self,product_id,content_byte_list,callername):
		...
		for case in cases:
			...
			status,caseo.description=self._hanlde_repeat_name("description=%s"%case.get('description'),'Case',flag)
		...
		for step in steps:
			...
			status,stepo.description=self._hanlde_repeat_name("description=%s"%step.get('description'),'Step',flag)
		...
		for businessdata in businessdatas:
			...
			status,bd.businessname=self._hanlde_repeat_name("businessname=%s"%businessdata.get('businessname'),'BusinessData',flag)
		...
		for con in dbcons:
			...
			status,cono.description=self._hanlde_repeat_name("description=%s"%con.get('description'),'DBCon',flag)
			if status is not 'success':

...
def _get_step_params(paraminfo):
    ...
    def _next(cur, parent=None, key=None):
        # print('_next')
...
class DataMove:
    ...
    def import_plan(self, product_id, content_byte_list, callername):
        ...
        for con in dbcons:
            ...
            cono.scheme = scheme
            try:
                authorlist = list(User.objects.filter(name=con.get('name')))
                if len(authorlist>0):
                    cono.author = authorlist[0]
                    cono.save()
                else:
                    cono.author=User.objects.get(name='admin')
                    cono.save()
            except:
                pass
                # print(traceback.format_exc())

...
class MessageParser(object):
    ...
    def _get_f_value(self,fcode)->tuple:
        ...
        if kind is None or kind=='separator':
            ...
            return ('success',text[start-1:end])
    ...
    def _compute_expression(self,child_exp:str):
        ...
        if count==1:
            fvalue=self._get_f_value(child_exp.split('=')[0])
            if fvalue[0] is not 'success':
                return fvalue
            res=''' '%s'%s'%s'  '''%(fvalue[1],'==',child_exp.split('=')[1])

        elif child_exp.__contains__('$'):
            fvalue=self._get_f_value(child_exp.split('$')[0])
            if fvalue[0] is not 'success':
                return fvalue
            res=''' '%s'.__contains__('%s') '''%(fvalue[1],child_exp.split('$')[1])

        
        else:
            for _a in self._SYMBOL:
                if child_exp.__contains__(_a):
                    fvalue=self._get_f_value(child_exp.split(_a)[0])
                    if fvalue[0] is not 'success':
                        return fvalue
                    res=''' '%s'%s'%s'  '''%(fvalue[1],_a,child_exp.split(_a)[1])

            res='非法比较符'

        try:
            er=eval(res)
            return('success',er) if er is True else ('fail',er)
...
if __name__=='__main__':
    ...
    #mp=MessageParser(f,c,e)
    mp1=MessageParser(f1,c,e)
   # res=mp.compute()
    res=mp1.compute()
    print(res)
...
def querytreelist(request):
	from .cm import weight_decorate,get_search_match
	datanode=[]
		...
		if type=='product':
			...
			for case in cases:
				datanode.append({
					'id':'case_%s'%case.id,
					'pId':'plan_%s'%plan.id,
					'name':weight_decorate('plan_case',case.id),
					'type':'case',
			...
			for step in steps:
				...
				'id':'step_%s'%step.id,
				'pId':'case_%s'%case.id,
				'name':weight_decorate('case_step',step.id),
				'type':'step',
			...
			for case0 in cases:
				data.append({
					'id':'case_%s'%case0.id,
					'pId':'case_%s'%idx,
					'name':weight_decorate('case_case',case0.id),
					'type':'case',
					'textIcon':'fa fa-folder',
					})
			...
			for business in businesslist:
				datanode.append({
					...
					'pId':'step_%s'%step.id,
					'name':weight_decorate('step_business',business.id),
					'type':'business',
					'textIcon':'fa fa-leaf',
					# 'open':True
					})
